...
其实就是各种语言中实现的 clone 方法，需要注意的是深克隆与浅克隆。
...
结构型（Structural）模式描述如何实现类或对象的组合。实现方法上而言，适配器、装饰器和代理模式都是切面编程（AOP）不同实现的应用。

## 适配器（Adapter）模式 - AOP


对既有的对象进行二次封装使其可以具有不同名但相同功能的另一种接口。
...
通过享元工厂类控制，将需要重用的对象的共同点与不同点区分开，共同点通过缓冲池缓冲，不同点通过方法的参数传入。个人理解，享元这个词是“共享元对象”的简写，这个模式其实就是我们常用的各种对象复用的池化方法。
...
这种情况违反了迪米特法则，我们可以通过引入一个中介者来降低耦合，其实此处思路和门面模式、命令模式类似：
...
- 《[Java设计模式](https://book.douban.com/subject/30173863/)》及其对应[博客](https://blog.csdn.net/lovelion/article/details/17517213)
