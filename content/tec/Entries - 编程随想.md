...
# 公共参数
MVC 中的每一层只关心自己的参数，尽量避免有某些参数同时会被所有层使用，这样极不利于解耦。如果真的有这种需求，应当抽出来一个公共变量放到第三方中，而不是通过绑定在请求上下文中的方式一级一级下传。
...
# 依赖注入思想很重要
对于类成员，不直接在类内进行实例化，而是通过初始化方法或 set 方法单独传入，这样可以随意组合组件，根据不同的场景使用不同的组件，大幅度解耦。


在web服务开发过程中，最重要的其实就是根据运营环境的不同注入不同的组件，例如单元测试时的logger为console，但是线上运行需要变更为FileLogger。
...
public class MetaData {
    ...
    String channelId;
    String versionId = IdUtil.genUuid();
