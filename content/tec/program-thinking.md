...
## 没有银弹
针对场景选用合适的工具。不仅仅是编程，也包括生活。例如数据库选型、开发工具、语言库等等。
...
计算机领域最重要解耦方法就是分层封装，例如网络的分层、硬件-操作系统-JVM-字节码、软件开发中的三层模型等等，都是下层对上层屏蔽了很多细节，只暴露有限的接口，提供了可替换的下层实现。这样的设计中，底层变化不会影响上层应用。
...
![](assets/2021-07-08-gitlab-dev-lifecycle.png)

上述循环中的开发阶段对应到 DevOps 上可以细化为如下过程：

![](assets/2021-07-08-gitlab-devops.png)
...
> 有一件关于估算的有趣的事——你使用的单位会对结果的解释产生影响。如果你说某件事需要130个工作日完成，那么听的人往往觉得实际要的时间会很接近这个数字。然而，如果你说的是“哦，大约6个月吧”，他们就会认为还需要5到7个月不等。两个数字表示的时间周期是一致的，但是“130天”却可能暗示了比你想象得更高的精度级别。
>
...
服务上线应该配置对服务及其依赖服务（DB、Redis等）的硬件监控，包括但不仅限于：
...
此外，还应从接口层面实现**黑盒监控**与**白盒监控**。黑盒监控依赖于 K8s 提供的健康监测能力即可，配合 K8s 能力保证服务存活。白盒监控依赖于程序内部提供接口相应信息，监控平台再根据原始信息进一步得到请求量、p99响应时间、异常率等指标。
