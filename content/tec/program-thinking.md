...
自底向上思考指的是考虑这个接口本质上哪些部分属于领域对象的能力、哪些部分属于服务层集成的外部能力，将不同能力划分清楚。

自顶向下开发指的是先实现接口定义，再利用 IDE 自动补全与填充的能力把从上至下的层级方法一层层补全，例如先实现 `controller`，然后利用 IDE 补全 `service` / `logic` / `dao`。
...
本质上讲，架构清晰是要隔离表现层和模型层，保持清晰的层级边界。
