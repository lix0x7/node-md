...
派生指的是B由A派生，有一些东西不变，有一些东西发生了改变，当使用派生对象上的内容时，沿派生链路寻找可以找到一个可用内容，这是典型的派生实现。从语言设计的角度看，JS所使用的原型链就是一种链式实现的派生；编程的角度看，派生的体现是继承或组合；从数据的角度看，层次化的配置文件是一种数据派生的体现，例如spring中的 application-test <- application.yaml。
...
服务上线应该配置对服务进行监控并配置告警，根据黄金指标（延迟、流量、错误、饱和度），监控范围应包括但不仅限于：
